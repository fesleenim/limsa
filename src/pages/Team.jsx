...
function Team() {
  ...
  return (
    <div className="max-w-[1420px] mx-auto p-4 md:mt-20 mt-15">
      <h1 className='md:text-5xl mt-10 text-3xl w-[75%] text-white mx-auto font-bold text-center'>{t('team.titel')}</h1>
      <p className='md:text-5xl text-3xl text-center text-[#6c2dba] font-bold mt-8'>{t('team.desc')}</p>
        ...
        {team.map((team, index) => (
          <div key={index} className="bg-[#121212] p-9 rounded-lg flex flex-col justify-baseline hover:scale-105 transition-transform">
            <div data-aos="fade-down"
              ...
              <div className='w-[75%]'>
                <strong className='text-white mt-4 text-xl'>{t(`team.title${index + 1}`)}</strong>
                <p className="mt-2 text-gray-500 text-sm">{t(`team.desc${index + 1}`)}</p>
              </div>
            </div>
          </div>
        ))}
