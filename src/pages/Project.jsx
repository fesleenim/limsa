    ...
    <div className="max-w-[1420px] mx-auto p-4 md:mt-20 mt-15">
      ...
      <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mt-10'>
        {projects.map((project, index) => (
          <a key={index} href={project.url} className="p-4 rounded-lg hover:scale-105 transition-transform">
            <strong className='text-white block mb-2'>{project.title}</strong>
            <img className="rounded-t-lg w-full h-48 object-cover" src={project.img} alt={project.title} />
            <h5 className="mt-3 text-lg font-normal text-gray-300">{project.desc}</h5>
          </a>
        ))}
      </div>
    </div>
